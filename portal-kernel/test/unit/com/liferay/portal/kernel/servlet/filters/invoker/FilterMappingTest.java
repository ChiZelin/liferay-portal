...
public class FilterMappingTest {
	...
	public void testConstructor() {
		_testConstructor(
			...
			urlRegexIgnorePattern -> Assert.assertNull(urlRegexIgnorePattern),
			urlRegexPattern -> Assert.assertNull(urlRegexPattern),
		...
		_testConstructor(
			...
			urlRegexIgnorePattern -> Assert.assertEquals(
				_URL_REGEX_IGNORE_PATTERN, urlRegexIgnorePattern.pattern()),
			urlRegexPattern -> Assert.assertEquals(
				_URL_REGEX_PATTERN, urlRegexPattern.pattern()),
	...
	public void testGetterMethods() {
		...
		Assert.assertEquals(_TEST_FILTER_NAME, filterMapping.getFilterName());
	...
	public void testIsMatch() {
		...
		_testLog(
			...
			Collections.singletonList(_URL_PATTERN),
			filterMapping -> filterMapping.isMatch(
				new MockHttpServletRequest(HttpMethods.GET, "/test/login.jsp"),
				Dispatcher.ASYNC, "/test/login.jsp"));
		_testLog(
			1,
			new String[] {
	...
	public void testIsMatchURLRegexPattern() {
		...
		_testIsMatchURLRegexPattern(
			true, "/test/login.jsp", null, new TestFilterConfig(null, null));
		_testIsMatchURLRegexPattern(
		...
		_testLog(
			...
			Collections.emptyList(),
			filterMapping -> filterMapping.isMatchURLRegexPattern(
				new MockHttpServletRequest(HttpMethods.GET, "/test/login.jsp"),
				"/test/login.jsp"));
		_testLog(
			1,
			...
			Collections.emptyList(),
			filterMapping -> filterMapping.isMatchURLRegexPattern(
				new MockHttpServletRequest(HttpMethods.GET, "/test/login.css"),
				"/test/login.css"));
	}
	...
	private void _testConstructor(
		Set<Dispatcher> expectedDispatchers,
		Consumer<Pattern> assertURLRegexIgnorePattern,
		Consumer<Pattern> assertURLRegexPattern, FilterConfig filterConfig,
		...
		Pattern urlRegexIgnorePattern = ReflectionTestUtil.getFieldValue(
			filterMapping, "_urlRegexIgnorePattern");
		Pattern urlRegexPattern = ReflectionTestUtil.getFieldValue(
		...
		assertURLRegexIgnorePattern.accept(urlRegexIgnorePattern);
		assertURLRegexPattern.accept(urlRegexPattern);
	}
	...
	private void _testLog(
		int expectedLogRecordsSize, String[] expectedMessages,
		List<String> urlPatterns, Consumer<FilterMapping> consumer) {

		try (CaptureHandler captureHandler =
				JDKLoggerTestUtil.configureJDKLogger(
					FilterMapping.class.getName(), Level.ALL)) {
			...
			consumer.accept(filterMapping);

			List<LogRecord> logRecords = captureHandler.getLogRecords();

			Assert.assertEquals(
				logRecords.toString(), expectedLogRecordsSize,
				logRecords.size());
