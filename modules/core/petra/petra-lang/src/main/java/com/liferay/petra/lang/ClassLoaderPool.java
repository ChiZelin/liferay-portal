...
public class ClassLoaderPool {
	...
	public static void register(String contextName, ClassLoader classLoader) {
		...
		if (contextName.lastIndexOf("_") > 0) {
			String[] str = contextName.split("_");
			...
			_fallbackClassLoaders.compute(
				...
				(key, classLoaders) -> {
					...
					classLoaders.put(new Version(str[1]), classLoader);
	...
	public static void unregister(ClassLoader classLoader) {
		...
		if (contextName != null) {
			...
			if (contextName.lastIndexOf("_") > 0) {
				...
				_unregisterFallback(str[0], new Version(str[1]));
			}
		}
	}
	...
	private static class Version implements Comparable<Version> {
		...
		private int _parseInt(String value, String version) {
			try {
		...
		private void _validate() {
			if (_major < 0) {
				throw new IllegalArgumentException(
					"invalid version \"" + _toString0() +
						"\": negative number \"" + _major + "\"");
			}

			if (_minor < 0) {
				throw new IllegalArgumentException(
					"invalid version \"" + _toString0() +
						"\": negative number \"" + _minor + "\"");
			}

			if (_micro < 0) {
				throw new IllegalArgumentException(
					"invalid version \"" + _toString0() +
						"\": negative number \"" + _micro + "\"");
			}

			for (char ch : _qualifier.toCharArray()) {
				if (('A' <= ch) && (ch <= 'Z')) {
					continue;
				}
